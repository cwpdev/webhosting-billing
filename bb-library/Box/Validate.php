...
class Box_Validate
{
    ...
    public function validateFileUpload($temp_name, $file_name, $max_size = 5, 
        $allowed_ext = ['pdf', 'doc', 'docx', 'jpg', 'gif', 'png'], 
        $allowed_mimes = [
            'application/pdf', 
            'application/msword',
        ...
        $file_mime = mime_content_type($temp_name);
        $file_size = filesize($temp_name) / 1024 / 1024;
        return in_array($file_ext, $allowed_ext) && in_array($file_mime, $allowed_mimes) && $file_size < $max_size;
