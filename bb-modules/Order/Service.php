...
class Service implements InjectionAwareInterface
{
    ...
    public function toApiArray(\Model_ClientOrder $model, $deep = true, $identity = null)
    {
        ...
        $data['service_data'] = $this->getOrderServiceData($model);
    ...
    public function getSearchQuery($data,$selectStmt = 'SELECT co.* ')
    {
        $query=$selectStmt;
        $query.=" from client_order co
    ...
    {
        $limit =  $this->di['array_get']($data, 'per_page', 10);
        list($sql, $params) = $this->getSearchQuery($data, "SELECT co.id,co.status, CONCAT(c.first_name,  ' ', c.last_name) as full_name");
        $sql = $sql.' LIMIT '.$limit;
        return $this->di['db']->getAssoc($sql, $params);
    }
     public function cryptConfigPassword($password){
        
        $key = openssl_digest($this->di['config']['locale_date_format'], 'SHA256', TRUE);
        $cipher = 'AES-128-CBC';
    ...
    public function decryptConfigPassword($cipherpass){ 
        $cipher = 'AES-128-CBC';
        $c = base64_decode($cipherpass);
    ...
    public function createOrder(\Model_Client $client, \Model_Product $product, array $data)
    {
        ...
        if(isset($config['password'])){
            $config['password'] = $this->cryptConfigPassword($config['password']);
            $config['password_secured'] = true;
        }
        ...
        if($product->type == 'hosting' && $activate){
            $config['server_id'] = $data['order_server_picked'];
        }
        //print_r($config); die;
