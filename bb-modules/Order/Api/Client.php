...
class Client extends \Api_Abstract
{
    ...
    public function get_domain_associated($config_raw){
        $config = json_decode($config_raw);
        if(property_exists($config, 'domain') && property_exists($config->domain, 'owndomain_sld') && property_exists($config->domain, 'owndomain_tld')){
            return '- ' . $config->domain->owndomain_sld . '.' . $config->domain->owndomain_tld;
        }
        return '';
    }
    public function get_logged_client_service_orders(){
        $bindings = array(
            ':client_id' => $this->di['session']->get('client_id')
        );
        return $this->di['db']->find('ClientOrder', 'client_id = :client_id', $bindings);
    }
    /**
     * Get order details
     *
