...
class Admin extends \Api_Abstract
{
    ...
    public function get_available_servers($config){
        $servers = $this->di['mod_service']('servicehosting')->getServersFullInfo();
        if(!array_key_exists('server_apply', $config) || $config['server_apply'] == 'all' || !array_key_exists('server_picked', $config)){
            return $servers;
        }else{
            $servers_result = [];
            foreach($servers as $key => $server){
                if(in_array($key, $config['server_picked'])){
                    $servers_result[$key] = $server;
                }
            }
    ...
    {
        $order = $this->_getOrder($data);
        //CWP To hanlde server on order
        if(array_key_exists('server', $data)){
            $config = $this->getService()->getConfig($order);
            $config['server_id'] = $data['server'];
            $this->getService()->updateOrderConfig($order, $config);
        }
        //CWP To hanlde server on order
