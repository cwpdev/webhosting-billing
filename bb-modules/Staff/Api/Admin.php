...
class Admin extends \Api_Abstract
{
    ...
    public function member_2fainfo(){
        $model = $this->di['db']->getExistingModelById('Admin', $this->getIdentity()->id, 'Staff member not found');
        return [
            'enabled' => $model->twofactor_enabled,
            'info' => $model->twofactor_enabled == 1 ? $this->get_2fainfo() : []
        ];
    }
    public function get_2fainfo(){
        $systemService =  $this->di['mod_service']('system');
        $company_name = $this->di['mod_service']('system')->getParamValue('company_name');
        $staff_member = $this->getIdentity();

        $aunthenticator_2fa = $this->di['2faA_authenticator'];
        $secret = $aunthenticator_2fa->generateRandomSecret();
        $qrCodeUrl = $aunthenticator_2fa->getQR('Admin area (' . $staff_member->email .')', $secret, $company_name);
        $this->di['session']->set('2fa_secret', $secret);
        return [
            'secret' => $secret,
            'image' => $qrCodeUrl
        ];
    }
    public function update_2fa($data){
        $required = array(
            'twofactor_status' => 'Two-Factor status is missing',
        );
        $this->di['validator']->checkRequiredParamsForArray($required, $data);
        //$model = $this->di['db']->findOne('Admin', 'id = ?', array($this->getIdentity()->id));
        $model = $this->di['db']->getExistingModelById('Admin', $this->getIdentity()->id, 'Staff member not found');
        //$model = $this->di['db']->getExistingModelById('Admin', 200003, 'Staff member not found');
        
        
        $model->twofactor_enabled = $data['twofactor_status'] == 'enabled' ? 1 : 0;
        $model->twofactor_secret = $this->di['session']->get('2fa_secret');
