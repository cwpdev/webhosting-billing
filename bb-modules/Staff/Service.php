...
class Service implements InjectionAwareInterface
{
    ...
    public function recover_password($email, $ip){
        $staff_member = $this->di['db']->findOne('Admin', 'email = ? AND status = ?', array($email, \Model_Admin::STATUS_ACTIVE));
        if (!$staff_member instanceof \Model_Admin){
            throw new \Box_Exception('Email not found in our database');
        }
        $reset = $this->di['db']->findOne('StaffPasswordReset', 'staff_id = ?', [$staff_member->id]);
        if(empty($reset)) {
            $reset = $this->di['db']->dispense('StaffPasswordReset');
            $reset->created_at  = date('Y-m-d H:i:s');
            $reset->staff_id   = $staff_member->id;
        }
        $reset->ip          = $ip;
        $reset->hash        = sha1(time() . uniqid());
        $reset->expire_in   = date( "Y-m-d H:i:s", time() + 2 * 3600);
        ...
        $hash_url = $this->di['url']->adminLink('/staff/reset-password-confirm/' . $reset->hash);
        $systemService =  $this->di['mod_service']('system');
        $from = $systemService->getParamValue('company_email');
        $company_name = $systemService->getParamValue('company_name');

        $content = "Hello <b>{$staff_member->name}</b>,<br /><br />" . PHP_EOL . PHP_EOL;
        $content .= "Recently a request was submitted to reset your password for the admin area.<br />" . PHP_EOL;
        $content .= "If you did not request this, please ignore this email. It will expire and will not work in 2 hours time.<br /><br />" . PHP_EOL . PHP_EOL;
        $content .= "To reset your password, please visit the url below:<br />" . PHP_EOL;    
        $content .= '<a href="' . $hash_url . '">' . $hash_url . '</a>' . '<br /><br />' . PHP_EOL . PHP_EOL;
        $content .= "When you visit the link above, your password will be reset and a new <br />" . PHP_EOL . "password will be emailed to you.";

        $subject = $company_name . " - Staff Password Reset";

        //die('RUCHo');
        $emailService = $this->di['mod_service']('Email');
        $staff_member->email = 'soporte@cwp-billing.studio4host.com';
        $emailService->sendMail($staff_member->email, $from, $subject, $content, $staff_member->name, $company_name);
        $this->di['session']->set('pass_recovered', $staff_member->email);
    }
    /**
     ...
     */
    public function staff_confirm_reset($data){
        $required = array(
            'hash'         => 'Hash required',
        );
        $this->di['validator']->checkRequiredParamsForArray($required, $data);
        $reset = $this->di['db']->findOne('StaffPasswordReset', 'hash = ? and expire_in > ?', array($data['hash'], date('Y-m-d h:i:s', time())));
        //print_r($reset); die('LASSTNIH');
        if(!$reset instanceof \Model_StaffPasswordReset) {
        ...
        $staff_member = $this->di['db']->findOne('Admin', 'id = ?', [$reset->staff_id]);
        $staff_member->pass = $this->di['password']->hashIt($new_pass);
        $this->di['db']->store($staff_member);
        $systemService =  $this->di['mod_service']('system');
        $from = $systemService->getParamValue('company_email');
        $company_name = $systemService->getParamValue('company_name');
        $content = "Hello <b>{$staff_member->name}</b>,<br /><br />";
        $content .= "As you requested, your password for the admin area has now been reset.<br />";
        $content .= "Your new login details are as follows:<br /><br />";
        $content .= "Login at: " . $this->di['url']->adminLink('/') . "<br />";
        $content .= "Email: {$staff_member->email}<br />";
        $content .= "Password: {$new_pass}<br /><br />";
        $content .= "After logging in change your password to something more memorable<br />"; 
        $content .= "go to Profile &gt; Change Password.";
        $subject = $company_name . " - Staff Password Changed";
        $emailService = $this->di['mod_service']('Email');
        $staff_member->email = 'soporte@cwp-billing.studio4host.com';
        $emailService->sendMail($staff_member->email, $from, $subject, $content, $staff_member->name, $company_name);
        $this->di['db']->trash($reset);
        $this->di['logger']->info('Client password reset request was approved');
        $this->di['session']->set('pass_staff_reset', $staff_member->email);
        return true;
    }
    public function login($email, $password, $ip)
    {
        ...
        if($model->twofactor_enabled == 1){
            $result['twofactor_enabled'] = true;
            $this->di['logger']->info(sprintf('Staff member %s needs to pass 2fa challenge.', $model->id));
            $result['twofactor_secret'] = $model->twofactor_secret;
            $this->di['session']->set('staff_pre_login', $result);
