...
class Admin extends \Api_Abstract
{
    ...
    {
        $service = $this->getService();
        if((!WHB_LICENSE && ($service->counter() < MAX_CLIENTS)) || (WHB_LICENSE)){
            $required = array(
    ...
    public function get_something($data){
        if($data['acc']=='getClients'){
            $orden = $this->di['db']->getAll('SELECT * FROM client_order WHERE id = '.$data['idOrden']);
            $return = $this->di['db']->getAll("SELECT id,first_name,last_name FROM client WHERE id != {$orden[0]['client_id']} AND status ='active' ORDER BY first_name");

            return (object)$return;
        }else if ($data['acc']=='newOwner'){
            if(is_numeric($data['idOrden']) && is_numeric($data['owner'])){
                $resp = $this->di['db']->exec("UPDATE client_order SET client_id = '{$data['owner']}' WHERE id = {$data['idOrden']}");
                return $resp;
            }
        }
    }
