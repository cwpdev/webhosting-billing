...
class Client implements \Box\InjectionAwareInterface
{
    ...
    public function get_tickets(\Box_App $app)
    {
        ...
        //$productsClient = $this->get_productsClient();
        return $app->render('mod_support_tickets');
    }
    public function get_productsClient(){
        //$api = $this->di['api_client'];
        //$client = $api->getIdentity();
        $service = $this->di['mod_service']('support'); 
        return $service->get_ClientOrder($this->di['session']->get('client_id'));
    }
