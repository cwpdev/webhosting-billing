...
class Admin extends \Api_Abstract
{
    ...
    public function batch_ticket_pipe($data)
    {
        $service = $this->di['mod_service']('support');
        $dptoHelps = $service->getDptoHelps();
        foreach ($dptoHelps as $dpto){
            $resp = $service->extractAndCreateTicket($dpto);
            print_r($resp);
        }
        return true;
    }
    ...
    public function helpdesk_update($data){
        $modStaff=implode(",", $data['staffs']);
        $data->staffs = $modStaff;
        $required = array(
            'id' => 'Help desk id is missing',
        );
        $this->di['validator']->checkRequiredParamsForArray($required, $data);
