...
class Service implements \Box\InjectionAwareInterface
{
    ...
    /** Logs from only tickets **/

    public function logsTickets($mensaje,$id_user,$id_ticket){

        $logs= $this->di['db']->dispense('TicketsLogs');
        $logs->action    = $mensaje;
        $logs->id_user   = $id_user;
        $logs->id_ticket = $id_ticket;
        $logs->created = date('Y-m-d H:i:s');

        return $this->di['db']->store($logs);
    ...
    public function closeTicket(\Model_SupportTicket $ticket, $identity)
    {
        ...
        if ($identity instanceof \Model_Admin) {
            $this->logsTickets('Ticket Close by Staff '.$identity->getFullName(),$identity->id,$ticket->id);
            ...
            $this->logsTickets('Ticket Close by Client ',0,$ticket->id);
    ...
    public function noteGetAuthorDetails(\Model_SupportTicketNote $model)
    {
        ...
        return array(
            'name'  => (isset($admin))?$admin->getFullName():'',
            'email' => $admin->email,
        );
    }
    ...
    public function messageGetAuthorDetails(\Model_SupportTicketMessage $model)
    {
        ...
        return array(
            'name'  => (isset($author))?$author->getFullName():'',
            'email' => $author->email,
        );
    }
    ...
    public function messageToApiArray(\Model_SupportTicketMessage $model)
    {
        ...
        $parsedAttachments = json_decode($data['attachment']);
        $data['attachment'] = is_array($parsedAttachments) ? $parsedAttachments : [];
        return $data;
    }
    ...
    public function ticketReply(\Model_SupportTicket $ticket, $identity, $content)
    {
        ...
        $msg->attachment = $this->manageMessageAttachments();
        $msg->created_at = date('Y-m-d H:i:s');
        $msg->updated_at = date('Y-m-d H:i:s');
        $msgId           = $this->di['db']->store($msg);

        if ($identity instanceof \Model_Admin) {
            $this->logsTickets('New Ticket Response by Staff '.$identity->getFullName(),$identity->id,$ticket->id);
            $ticket->status = \Model_SupportTicket::ONHOLD;
        } elseif ($identity instanceof \Model_Client) {
            $this->logsTickets('New Ticket Response by User',0,$ticket->id);
    ...
    public function ticketCreateForAdmin(\Model_Client $client, \Model_SupportHelpdesk $helpdesk, $data, \Model_Admin $identity)
    {
        ...
        $this->logsTickets('Ticket created for Staff '.$identity->getFullName(), $helpdesk->id, $ticket->id);
    ...
    public function ticketCreateForGuest($data)
    {
        ...
        $msg->updated_at          = date('Y-m-d H:i:s');
        $this->di['db']->store($msg);
        $this->logsTickets('Ticket created for Guest ', 0, $ticket->id);
    ...
    public function manageMessageAttachments(){
        $attachmentsList = [];
        foreach ($_FILES as $key => $file){
            if(preg_match('/^message_image_/', $key) && !empty($file['name']) && $this->di['validator']->validateFileUpload($file['tmp_name'], $file['name'])){ //tmp_name
                $ext = pathinfo($file['name'], PATHINFO_EXTENSION);
                $fileName = bin2hex(random_bytes(10)) . '.' . $ext;
                if(move_uploaded_file($file['tmp_name'], $this->di['config']['path_data'] . '/uploads/' . $fileName)){
                    $attachmentsList[] = [
                        'f' => $fileName,
                        'e' => $ext
                    ];
        ...
        return json_encode($attachmentsList);
    }
    public function ticketCreateForClient(\Model_Client $client, \Model_SupportHelpdesk $helpdesk, array $data)
    {
        ...
        $ticket->subject             = $data['subject'];
        $ticket->support_helpdesk_id = $helpdesk->id;
        $ticket->priority   = $this->searchPriority($data['id_product_client']);
        $ticket->id_product_client   = $data['id_product_client'];
        $ticket->created_at          = date('Y-m-d H:i:s');
        $ticket->updated_at          = date('Y-m-d H:i:s');
        ...
        $this->logsTickets('Ticket created for Client', 0, $ticket->id);
    ...
    private function searchPriority($id){
        if($id!=0){
            $query = "SELECT product.priority_support FROM client_order INNER JOIN product ON client_order.product_id = product.id WHERE client_order.id = {$id}";
            $data = $this->di['db']->getAll($query);
            return $data[0]['priority_support'];
        }else{
            return 25;
        }
    }
    ...
    public function helpdeskUpdate(\Model_SupportHelpdesk $model, $data)
    {
        ...
        $model->pipe_hostname = $this->di['array_get']($data, 'pipe_hostname', $model->pipe_hostname);
        $model->pipe_port = $this->di['array_get']($data, 'pipe_port', $model->pipe_port);
        $model->pipe_user = $this->di['array_get']($data, 'pipe_user', $model->pipe_user);
        $model->pipe_pass = $this->di['array_get']($data, 'pipe_pass', $model->pipe_pass);
        $model->pipe_delete_email = $this->di['array_get']($data, 'pipe_delete_email', $model->pipe_delete_email);
        $model->pipe_status = $this->di['array_get']($data, 'pipe_status', $model->pipe_status);

        $model->updated_at = date('Y-m-d H:i:s');
        $id                = $this->di['db']->store($model);

        $this->di['db']->exec("DELETE FROM  support_helpdesk_staff WHERE id_support_helpdesk = {$data['id']}");

        foreach($data['staffs'] as $staff){
            $mod = $this->di['db']->dispense('SupportHelpdeskStaff');
            $mod->id_support_helpdesk = $data['id'];
            $mod->id_staff = $staff;
            $this->di['db']->store($mod);
        }
    ...
    public function helpdeskCreate($data)
    {
        ...
        $model->pipe_hostname       = $this->di['array_get']($data, 'pipe_hostname', NULL);
        $model->pipe_port           = $this->di['array_get']($data, 'pipe_port', NULL);
        $model->pipe_user           = $this->di['array_get']($data, 'pipe_user', NULL);
        $model->pipe_pass           = $this->di['array_get']($data, 'pipe_pass', NULL);
        $model->pipe_delete_email   = $this->di['array_get']($data, 'pipe_delete_email', NULL);
        $model->pipe_status         = $this->di['array_get']($data, 'pipe_status', NULL);

        $model->created_at          = date('Y-m-d H:i:s');
        $model->updated_at          = date('Y-m-d H:i:s');
        //$model->id_product_client   = $this->di['array_get']($data, 'id_product_client', NULL);
        $id                         = $this->di['db']->store($model);

        $this->di['logger']->info('Created helpdesk #%s', $id);

        if(is_numeric($id) && $model->staffs!=''){
            $tz = explode(',', $model->staffs);
            foreach($tz as $staff){
                // insert
                //support_helpdesk_staff
            }
        }
    ...
    public function noteCreate(\Model_SupportTicket $ticket, \Model_Admin $identity, $note)
    {
        ...
        $this->logsTickets('Added note to ticket by Staff '.$identity->getFullName(),$identity->id,$ticket->id);
        return $id;
    }
    public function ticketTaskComplete(\Model_SupportTicket $model)
    {
        ...
        $model->updated_at = date('Y-m-d H:i:s');
        $id                = $this->di['db']->store($model);
        $this->logsTickets('Marked ticket task as complete by Staff',$identity->id,$ticket->id);
        $this->di['logger']->info('Marked ticket #%s task as complete', $id);

        return true;
    }
    public function get_ClientOrder($client_id){
        return $this->di['db']->getAll("SELECT * FROM client_order WHERE client_id ={$client_id}");
    }
    ...
    public function getDptoHelps(){
        return $this->di['db']->getAll("Select * FROM support_helpdesk");
    }
    public function pipe_delete_email($connection,$i){
        imap_delete($connection, $i);
        imap_expunge($connection);
    }
    public function pipe_create_ticket($data,$inbox){
        $client=$this->idClient($data,$inbox);
        if($data['pipe_only_clients']==1 && $client->id>0){
            $header = $inbox['header'];
            $body = $inbox['body'];
            ...
            $ticket->client_id           = $client->id;
            $ticket->status              = 'open';
            $ticket->subject             = $header->subject;
            $ticket->priority            = 25;
            $ticket->support_helpdesk_id = $data['id'];
            $ticket->created_at          = date('Y-m-d H:i:s');
            $ticket->updated_at          = date('Y-m-d H:i:s');
            $ticketId                    = $this->di['db']->store($ticket);
            ...
            $this->di['db']->store($msg);
            return (int)$ticketId;
        }elseif($data['pipe_only_clients']==0){
            $header = $inbox['header'];
            $body = $inbox['body'];
    ...
    public function idClient($data,$inbox){
        $header=$inbox['header'];
        $arrname = (array)$header->from;
        $name = $arrname[0]->personal;
        $email = $arrname[0]->mailbox.'@'.$arrname[0]->host;
        $resp = $this->di['db']->getAll("Select id,first_name,last_name FROM client WHERE email='{$email}'");
        if($data['pipe_only_clients']==1){
            if(count($resp)>0){
                $id = $resp[0]['id'];
                $name = $resp[0]['first_name'].' '.$resp[0]['last_name'];
                $type = 'registered';
            }else{
                $id = '';
                $name = '';
        ...
        return (object)['id'=>$id,'type'=>$type,'name'=>$name,'email'=>$email];
    }
    public function extractAndCreateTicket($data){
        if($data['pipe_status']==1){
            $authhost="{".trim($data['pipe_hostname']).":".trim($data['pipe_port'])."/pop3/notls}";
            if($data['pipe_hostname']!='' && $data['pipe_port']!=''&& $data['pipe_user']!='' && $data['pipe_pass']!=''){
                if ($connection=imap_open( $authhost, $data['pipe_user'], $data['pipe_pass'] )){
                    ...
                    if($num_msg>0){
                        for($i = 1; $i <= $num_msg; $i++) {
                            ...
                            $header = $inbox['header'];
                            if($header->subject!='Undelivered Mail Returned to Sender' && $header->Subject!='Undelivered Mail Returned to Sender'){
                                $ticket=$this->pipe_create_ticket($data,$inbox);
                                if($data['pipe_delete_email']==1 && is_numeric($ticket)){
                                    $this->pipe_delete_email($connection,$i);
                                }
                            }else{
                                $this->pipe_delete_email($connection,$i);
                            }
                    ...
                    $this->di['logger']->info('Connection error (Cron) #%s', '');
                    return "connection error";
                }
            }else{
                $this->di['logger']->info('Missing connection data (Cron) #%s', '');
                return "Missing connection data";
            }
        }
    }
}
