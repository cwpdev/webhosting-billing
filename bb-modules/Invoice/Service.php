...
class Service implements InjectionAwareInterface
{
    ...
    public function getSearchQuery($data,$selectStmt = 'SELECT p.* ')
    {
        $sql=$selectStmt;
        $sql.=" FROM invoice p LEFT JOIN invoice_item pi ON (p.id = pi.invoice_id) LEFT JOIN client cl ON (cl.id = p.client_id) WHERE 1 ";
        ...
        if($search) {
            $sql .= " AND (p.id = :int OR p.nr LIKE :search_like OR p.id LIKE :search OR pi.title LIKE :search_like OR  cl.first_name like :search_like OR cl.last_name like :search_like)";
    ...
    public function getPairs($data)
    {
        ...
        list($sql, $params) = $this->getSearchQuery($data, "SELECT p.id,p.status, CONCAT(cl.first_name,  ' ', cl.last_name) as full_name");
        $sql = $sql.' LIMIT '.$limit;
