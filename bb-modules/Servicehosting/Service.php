...
class Service implements InjectionAwareInterface
{
    ...
    public function validateOrderData(array &$data)
    {
        ...
        if(!isset($data['hosting_plan_id'])) {
            throw new \Box_Exception('Hosting product is not configured completely. Configure hosting plan for hosting product. '.json_encode($data), null, 702);
        }
    ...
    public function action_create(\Model_ClientOrder $order)
    {
        ...
        $model->service_hosting_server_id = $c['server_id'];
    ...
    public function resendWelcomeEmail(\Model_ClientOrder $order){
        $serviceOrder = $this->di['mod_service']('order');

        try {
            //$order = $this->di['db']->getExistingModelById('ClientOrder', $order_id, 'Order not found');;
            $s  = $serviceOrder->getOrderServiceData($order);
            $orderArr = $serviceOrder->toApiArray($order, true);

            $email = [];
            $email['to_client'] = $order->client_id;
            $email['code'] = 'mod_servicehosting_activated';
            $email['service'] = $s;
            ...
            throw new \Box_Exception('Email message not sent.');
        }
    }
    public function action_activate(\Model_ClientOrder $order)
    {
        ...
        if(isset($c['password']) && !empty($c['password'])) {
            $pass_crypted = $c['password'];
            $pass = isset($c['password_secured']) && $c['password_secured'] === true ? $orderService->decryptConfigPassword($c['password']) : $c['password'];
        }else{
            $pass_crypted = $orderService->cryptConfigPassword($pass);
        }
    ...
    public function getQuota(\Model_ClientOrder $order, \Model_ServiceHosting $model, $data){
        list($adapter, $account) = $this->_getAM($model);

        return $adapter->getQuota($account);
    }
    ...
    public function changeAccountPassword(\Model_ClientOrder $order, \Model_ServiceHosting $model, $data)
    {
        ...
        $model->pass = $this->di['mod_service']('order')->cryptConfigPassword($p);
        $model->updated_at = date('Y-m-d H:i:s');
        $this->di['db']->store($model);
    ...
    public function getServersFullInfo()
    {
