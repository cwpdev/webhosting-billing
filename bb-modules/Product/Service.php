...
class Service implements InjectionAwareInterface
{
    ...
    public function toApiArray(\Model_Product $model, $deep = true, $identity = null)
    {
        ...
        $result = array(
            ...
            'priority_support'      => $model->priority_support,
        );
    ...
    public function updateProduct(\Model_Product $model, $data)
    {
        ...
        if (isset($data['config']) && is_array($data['config'])) {
            ...
            if($data['config']['server_apply'] == 'all'){
                $data['config']['server_picked'] = [];
            }
        ...
        $model->priority_support    = $this->di['array_get']($data, 'priority_support', $model->setup);
    ...
    public function createAddon($title, $description = null, $setup = null, $status = null, $iconUrl = null,$priority_support = '25')
    {
        $modelPayment       = $this->di['db']->dispense('ProductPayment');
        $modelPayment->type = \Model_ProductPayment::FREE;
        $paymentId          = $this->di['db']->store($modelPayment);
        ...
        $model->is_addon            = 1;
        $model->priority_support    = $priority_support;
