...
class Admin extends \Api_Abstract
{
    ...
    {
        $service = $this->getService();
        if((!WHB_LICENSE && ($service->counter() < MAX_PRODUCTS)) || (WHB_LICENSE)) {
            $required = array(
    ...
    public function addon_create($data)
    {
        ...
        $priority_support = $this->di['array_get']($data, 'priority_support', null);
        $service = $this->getService();
        return $service->createAddon($title, $description, $setup, $status, $iconUrl,$priority_support);
    }
